...
dat<-read.table(finput,header = 1,stringsAsFactors = F)
...
for(g in genes){
  ...
  for(score in unique(gdat$CADD) ){
    ...
    for(i in 1:dim(test)[1]){
      ...
      info<-rbind(info,c(g,score,f_het,n,phet))
